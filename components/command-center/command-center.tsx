  ...
  return <main id="negosu-command-center-page" className="mx-auto min-w-0 max-w-7xl pb-5">
    ...
    <section id="negosu-command-center-metrics" aria-label="Business metrics" className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-5">
      {snapshot.metrics.map((metric) => {
        const content = <><span className="text-[11px] font-bold uppercase tracking-wide text-slate-500 sm:text-xs">{metric.label}</span><strong className="mt-1 block text-xl font-black tracking-tight text-admin-text sm:text-2xl">{metric.valueKind === "currency" ? formatCurrency(metric.value, snapshot.scope.currency) : metric.value.toLocaleString("en-PH")}</strong>{metric.helperText ? <small className="mt-0.5 hidden text-xs text-slate-500 sm:block">{metric.helperText}</small> : null}</>;
    ...
    <div className="mt-3 grid min-w-0 gap-3 xl:grid-cols-[1.05fr_0.95fr]">
      <section id="negosu-action-inbox" aria-labelledby="negosu-action-inbox-title" className="min-w-0 rounded-xl border border-admin-border border-l-4 border-l-brand-primary bg-admin-surface p-4">
        <div className="flex items-start justify-between gap-3"><div><h2 id="negosu-action-inbox-title" className="font-black text-admin-text">Action Inbox</h2><p className="text-xs text-slate-500">Live conditions disappear when the underlying work is resolved.</p></div><Badge>{attentionCount} {attentionCount === 1 ? "condition" : "conditions"}</Badge></div>
      ...
      <section id="negosu-today-operations" aria-labelledby="negosu-today-operations-title" className="min-w-0 rounded-xl border border-admin-border bg-admin-surface p-4">
        <div className="flex items-start justify-between gap-3"><div><h2 id="negosu-today-operations-title" className="font-black text-admin-text">{todayTitle}</h2><p className="text-xs text-slate-500">{todayDescription}</p></div><CalendarDays aria-hidden="true" className="text-brand-primary" size={19}/></div>
