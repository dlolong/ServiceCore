...
export function JobOrderWorkTracking({
  ...
  return <Card id="job-order-work-tracking-section" className="p-4 sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="font-black">Technician work tracking</h2><p className="mt-1 text-xs text-zinc-500">Operational labor effort from server timestamps. It does not change billing or payroll.</p></div>
    ...
    <div id="job-order-active-technicians" className="mt-4 grid gap-2">
      {summary.activeSessions.map((session) => <article id={`technician-work-session-row-${session.id}`} key={session.id} className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2"><span><strong>{session.technicianName}</strong><small className="block text-emerald-800">Working now</small></span><WorkSessionTimer id={`job-order-active-session-timer-${session.id}`} startedAt={session.startedAt} className="font-black text-emerald-800"/></div>
