...
select is(to_regprocedure('public.create_first_organization(text,text,text,text)')::text, null, 'onboarding RPC accepts no browser role argument');
...
select throws_ok($$insert into public.appointments (organization_id, branch_id, customer_id, vehicle_id) values ('20000000-0000-4000-8000-000000000002', '40000000-0000-4000-8000-000000000002', '50000000-0000-4000-8000-000000000002', '60000000-0000-4000-8000-000000000002')$$, '42501', 'new row violates row-level security policy for table "appointments"', 'Owner A cannot insert Appointment B records');
select throws_ok($$insert into public.job_orders (organization_id, branch_id, customer_id, vehicle_id) values ('20000000-0000-4000-8000-000000000002', '40000000-0000-4000-8000-000000000002', '50000000-0000-4000-8000-000000000002', '60000000-0000-4000-8000-000000000002')$$, '42501', 'new row violates row-level security policy for table "job_orders"', 'Owner A cannot insert Job B records');
select throws_ok($$insert into public.job_order_items (organization_id, job_order_id, service_name_snapshot, unit_price_centavos, line_total_centavos) values ('20000000-0000-4000-8000-000000000002', 'a0000000-0000-4000-8000-000000000002', 'Blocked', 0, 0)$$, 'P0001', 'Job item organization mismatch', 'Owner A cannot insert Job B items');
select throws_ok($$insert into public.payments (organization_id, branch_id, amount_centavos, method) values ('20000000-0000-4000-8000-000000000002', '40000000-0000-4000-8000-000000000002', 1, 'cash')$$, 'P0001', 'Payment/branch organization mismatch', 'Owner A cannot insert Payment B records');
select throws_ok($$insert into public.inventory_items (organization_id, branch_id, name) values ('20000000-0000-4000-8000-000000000002', '40000000-0000-4000-8000-000000000002', 'Blocked')$$, 'P0001', 'Inventory item/branch organization mismatch', 'Owner A cannot insert Inventory B records');
select throws_ok($$insert into public.inventory_movements (organization_id, branch_id, inventory_item_id, movement_type, quantity_delta) values ('20000000-0000-4000-8000-000000000002', '40000000-0000-4000-8000-000000000002', 'c0000000-0000-4000-8000-000000000002', 'stock_in', 1)$$, 'P0001', 'Inventory movement organization mismatch', 'Owner A cannot insert Inventory B movements');
...
select is_empty($$update public.job_orders set internal_note = 'blocked' where id = 'a0000000-0000-4000-8000-000000000002' returning 1$$, 'Owner A cannot update Job B');
select is_empty($$update public.payments set reference = 'blocked' where id = 'b0000000-0000-4000-8000-000000000002' returning 1$$, 'Owner A cannot update Payment B');
...
select is_empty($$delete from public.job_orders where id = 'a0000000-0000-4000-8000-000000000002' returning 1$$, 'Owner A cannot delete Job B');
select is_empty($$delete from public.payments where id = 'b0000000-0000-4000-8000-000000000002' returning 1$$, 'Owner A cannot delete Payment B');
...
select lives_ok($$select public.create_first_organization('New Shop', 'fixture-new-shop', '+63 900 000 0000')$$, 'authenticated user can atomically onboard');
...
select is((select count(*) from public.branches where name = 'Main Branch')::bigint, 1::bigint, 'onboarding creates the main branch');
...
select throws_ok($$select public.create_first_organization('Second Shop', 'fixture-second-shop', null)$$, 'P0001', 'User already belongs to an organization', 'onboarding cannot be repeated');
...
select throws_ok($$insert into public.inventory_items (organization_id, branch_id, name) values ('20000000-0000-4000-8000-000000000001', '40000000-0000-4000-8000-000000000002', 'Mismatch')$$, 'P0001', 'Inventory item/branch organization mismatch', 'inventory item/branch tenant mismatch is rejected');
