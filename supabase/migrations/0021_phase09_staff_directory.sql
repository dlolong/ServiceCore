-- Phase 09 trusted staff directory; auth emails remain unavailable to ordinary table queries.
create function public.list_staff(p_organization_id uuid)
...
language sql stable security definer set search_path=public,auth,pg_temp as $$
  select m.id,m.user_id,coalesce(p.full_name,u.email,'ServiceCore user'),u.email,m.role,m.is_active,
