...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = publicShop.description ?? `Book automotive services with ${publicShop.name}.`;
  return {
    title: `${publicShop.name} | ${verticalBrands.automotive.displayName}`,
    description,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="public-automotive-shop-page" className="min-h-screen bg-slate-50 text-admin-text">
    ...
    <section id="public-automotive-shop-hero" className="relative overflow-hidden bg-brand-ink text-white">
      <div className="mx-auto grid min-h-[420px] max-w-7xl items-center gap-8 px-5 py-16 lg:grid-cols-2">
        ...
        <div className="relative">
          ...
          <p className="font-bold text-blue-300">Automotive care powered by {verticalBrands.automotive.displayName}</p>
          <h1 className="mt-3 text-4xl font-black sm:text-6xl">{publicShop.name}</h1>
          <p className="mt-5 max-w-2xl text-lg text-zinc-200">{publicShop.description ?? "Professional automotive care, conveniently booked online."}</p>
          <Button id="public-automotive-shop-book-button" asChild className="mt-7"><Link href={`/shop/${slug}/book`}>Request a booking</Link></Button>
        </div>
      </div>
    </section>
