...
export async function saveCustomer(data: FormData) {
  const id = formValue(data, "id"); const parsed = customerSchema.safeParse(customerInput(data)); const back = id ? `/dashboard/customers/${id}/edit` : "/dashboard/customers/new";
  if (!parsed.success) message(back, "error", firstError(parsed.error));
  ...
  if (!values.acceptDuplicate && (normalized || values.email)) {
    ...
    if (duplicate) redirect(`${back}?warning=${encodeURIComponent(`Possible duplicate: ${duplicate.full_name}`)}&duplicateId=${duplicate.id}`);
  }
  ...
  revalidatePath("/dashboard/customers"); redirect(`/dashboard/customers/${result.data.id}?message=${encodeURIComponent(`Customer ${id ? "updated" : "created"}.`)}`);
}
...
export async function saveVehicle(data: FormData) {
  const id = formValue(data, "id"); const parsed = vehicleSchema.safeParse(vehicleInput(data)); const back = id ? `/dashboard/vehicles/${id}/edit` : "/dashboard/vehicles/new";
  if (!parsed.success) message(back, "error", firstError(parsed.error));
  ...
  if (!values.acceptDuplicate && (plate || values.vin)) {
    ...
    const { data: duplicate } = filters ? await query.or(filters).maybeSingle() : { data: null };
    if (duplicate) redirect(`${back}?warning=${encodeURIComponent(`Possible duplicate: ${duplicate.make} ${duplicate.model}`)}&duplicateId=${duplicate.id}`);
  }
  ...
  revalidatePath("/dashboard/vehicles"); redirect(`/dashboard/vehicles/${result.data.id}?message=${encodeURIComponent(`Vehicle ${id ? "updated" : "created"}.`)}`);
}
