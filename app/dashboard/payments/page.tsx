...
export default async function Page() {
  ...
  return <main id="payments-page" className="mx-auto min-w-0 max-w-6xl">
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">
      <section id="payments-by-method" className="rounded-ui-lg border border-admin-border bg-white p-5"><h2 className="font-black text-admin-text">By payment method</h2><div className="mt-3 divide-y divide-slate-100">{byMethod.map(([method, total]) => <div id={`payments-method-${method}`} className="flex justify-between py-3 text-sm capitalize" key={method}><span>{method.replaceAll("_", " ")}</span><strong>{formatMoney(total)}</strong></div>)}{!byMethod.length && <p id="payments-by-method-empty" className="py-6 text-sm text-zinc-500">No payments today.</p>}</div></section>
      <section id="payments-outstanding-invoices" className="rounded-ui-lg border border-admin-border bg-white p-5"><h2 className="font-black text-admin-text">Outstanding invoices</h2><div className="mt-3 divide-y divide-slate-100">{invoices?.map(invoice => <Link id={`payments-invoice-${invoice.id}`} className="flex min-h-14 items-center justify-between gap-3 py-3 text-admin-text hover:text-brand-primary-strong" href={`/dashboard/invoices/${invoice.id}`} key={invoice.id}><span className="min-w-0"><strong>{invoice.invoice_number}</strong><small className="block truncate text-zinc-500">{invoice.customer_name_snapshot}</small></span><strong className="shrink-0">{formatMoney(invoice.balance_centavos)}</strong></Link>)}{!invoices?.length && <p id="payments-outstanding-empty" className="py-6 text-sm text-zinc-500">No outstanding invoices.</p>}</div></section>
    </div>
  </main>;
}
