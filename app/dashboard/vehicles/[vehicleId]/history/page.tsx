...
export default async function Page({params}:{params:Promise<{vehicleId:string}>}){
  ...
  return <main id="vehicle-history-page" className="mx-auto max-w-6xl">
    <header id="vehicle-history-page-header" className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold text-brand-primary">Vehicle history</p><h1 className="text-3xl font-black">{vehicleLabel(vehicle)}</h1><p className="mt-1 text-sm text-zinc-500">Current odometer: {vehicle.odometer_km==null?"Not captured":`${vehicle.odometer_km.toLocaleString("en-PH")} km`}</p></div><Button id="vehicle-history-back-button" asChild variant="secondary"><Link href={`/dashboard/vehicles/${vehicle.id}`}>Vehicle overview</Link></Button></header>
