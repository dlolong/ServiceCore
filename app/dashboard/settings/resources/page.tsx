...
export default async function SchedulingResourcesPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; create?: string }> }) {
  ...
  return <main id={salon ? "salon-resources-page" : "scheduling-resources-page"} className="mx-auto min-w-0 max-w-6xl">
    ...
    <FormMessage {...params}/>
    {canManage&&!salon&&<Card className="mt-6 p-5"><h2 className="font-black">Add service bay or resource</h2>{form}</Card>}
...
function ResourceCard({resource,canManage,salon=false}:{resource:ResourceRow;canManage:boolean;salon?:boolean}) { const prefix=salon?"salon-resource":"scheduling-resource"; return <Card id={`${prefix}-card-${resource.id}`} elevation="none" className="flex flex-wrap items-center justify-between gap-3 p-5"><div><h2 className="font-black">{resource.name}</h2><p className="text-sm text-zinc-500">{branchName(resource)} · {resource.resource_type} · capacity {resource.capacity}{resource.is_active?"":" · inactive"}</p></div>{canManage&&<ResourceActions resource={resource} context={salon?"mobile":"card"} prefix={prefix}/>}</Card>; }
