...
export async function transitionSalonAppointment(data:FormData){
  ...
  try{await requireSalonOperator();}catch(error){go(id,"error",reportActionError("salon.appointment.authorize_transition",error,"Unable to update the appointment."));}
...
export async function createSalonAppointmentLink(data:FormData){
  ...
  try{await requireSalonOperator();}catch(error){go(id,"error",reportActionError("salon.appointment.authorize_link",error,"Unable to create the customer link."));}
  const expiresAt=new Date(Date.now()+14*86_400_000).toISOString();let result;try{result=await createAppointmentSelfServiceLink({appointmentId:id,expiresAt});}catch(error){go(id,"error",reportActionError("salon.appointment.create_link",error,"Unable to create the customer link."));}
...
export async function recordSalonAppointmentPayment(data:FormData){
  ...
  const{activeMembership}=await getDashboardContext();if(activeMembership.industry!=="salon"||!["owner","manager","cashier"].includes(activeMembership.role))go(id,"error","Payment access required.");let validated;try{validated=recordPaymentInputSchema.parse({appointmentId:id,amountCentavos:Number(amount),method:parsed.data.method,idempotencyKey:parsed.data.idempotencyKey,reference:parsed.data.reference||null,notes:parsed.data.notes||null});}catch(error){go(id,"error",reportActionError("salon.payment.validate",error,"Unable to record the payment."));}const supabase=await createClient();const{error}=await supabase.rpc("record_appointment_payment",{p_appointment_id:validated.appointmentId,p_amount_centavos:validated.amountCentavos,p_method:validated.method,p_idempotency_key:validated.idempotencyKey,p_reference:validated.reference,p_notes:validated.notes});if(error)go(id,"error",error.message.includes("amount")?"The payment exceeds the current balance.":error.message.includes("Idempotency key conflicts")?"This payment submission conflicts with an earlier request. Refresh and try again.":"Unable to record payment.");
